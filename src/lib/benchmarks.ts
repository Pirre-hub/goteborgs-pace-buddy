...
function riegelProjectTimeSec(timeSec: number, fromDistanceKm: number, toDistanceKm: number) {
  return timeSec * Math.pow(toDistanceKm / fromDistanceKm, RIEGEL_EXPONENT);
}

// Best performance from runs in 3–21 km range, average of top 5 half-marathon projections.
// Uses Riegel's formula so short-run pace is not treated as sustainable for 21.1 km.
...
export function bestRecentPaceSecPerKm(runs: Run[]): {
  ...
  date: string;
  sampleSize: number;
  projectedHalfMarathonSec: number;
} | null {
  const candidates = runs
    ...
    .map((r) => ({
      ...
      projectedHalfMarathonSec: riegelProjectTimeSec(
        r.moving_time,
        r.distance / 1000,
        HALF_MARATHON_KM,
      ),
    }))
    .sort((a, b) => a.projectedHalfMarathonSec - b.projectedHalfMarathonSec);
  ...
  const projectedHalfMarathonSec = top.reduce((s, r) => s + r.projectedHalfMarathonSec, 0) / top.length;
  return {
    pace: avgPace,
    distance_km: avgDist,
    date: top[0].date,
    sampleSize: top.length,
    projectedHalfMarathonSec,
  };
}
...
export function calcAgeGrade(finishTimeSec: number, age: number): {
  ...
  if (percent >= 90) { tier = "Exceptionellt hög nivå"; tone = "excellent"; }
  else if (percent >= 80) { tier = "Mycket hög nivå"; tone = "excellent"; }
  else if (percent >= 70) { tier = "Stark nivå"; tone = "good"; }
  else if (percent >= 60) { tier = "Bra motionsnivå"; tone = "good"; }
  else if (percent >= 50) { tier = "Över medel"; tone = "average"; }
